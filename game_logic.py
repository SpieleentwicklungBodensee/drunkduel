...
def _handle_player_hit(bullet, hit_player_index):
    ...
    # Debug output
    print(f"Player {hit_player_index + 1} took {damage} damage! Health: {hit_player.health}")
    
    if is_dead:
        # Player died - give point to shooter and reset
        other_player.score += 1
        
        # Give shooter some ammo back as reward
        other_player.ammo = min(other_player.ammo + 2, 6)
        
        # Reset hit player position and health
        if hit_player_index == 0:  # Player 1 hit
            hit_player.xpos = 2 * TILE_WIDTH
            hit_player.ypos = 2 * TILE_HEIGHT
        else:  # Player 2 hit
            hit_player.xpos = 13 * TILE_WIDTH
            hit_player.ypos = 13 * TILE_HEIGHT
            
        # Reset player stats
        hit_player.reset_health()
        hit_player.ammo = 4
        
        # Reset alcohol level for hit player (teilweise)
        hit_player.alcohol_level = max(0, hit_player.alcohol_level - 0.3)  # Schock nüchtert etwas auf

        # Check for victory condition (first to 5 points wins)
        if other_player.score >= 5:
        ...
        # Small alcohol reduction from shock
        hit_player.alcohol_level = max(0, hit_player.alcohol_level - 0.1)
